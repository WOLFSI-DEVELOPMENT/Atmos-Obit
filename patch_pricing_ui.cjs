...
const pricingCode = `import React, { useEffect, useState } from 'react';
    ...
    <div className="w-full min-h-screen bg-[#000000] text-white font-sans selection:bg-neutral-800 pb-32">
      ...
      <main className="max-w-[1200px] mx-auto px-6 pt-32">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-indigo-500/10 text-indigo-400 text-[14px] font-medium mb-6 border border-indigo-500/20">
            <Tag size={14} />
            <span>Pricing</span>
          </div>
          ...
          {/* Toggle */}
          <div className="inline-flex items-center p-1 bg-[#111111] border border-white/10 rounded-full">
            <button
              onClick={() => setIsYearly(false)}
              className={\`px-6 py-2.5 rounded-full text-[15px] font-medium transition-all \${!isYearly ? 'bg-[#222222] text-white shadow-lg border border-white/5' : 'text-[#8a8a8a] hover:text-white'}\`}
            >
              Monthly
            </button>
            <button
              onClick={() => setIsYearly(true)}
              className={\`px-6 py-2.5 rounded-full text-[15px] font-medium transition-all flex items-center gap-2 \${isYearly ? 'bg-[#222222] text-white shadow-lg border border-white/5' : 'text-[#8a8a8a] hover:text-white'}\`}
            >
              Yearly
              <span className="px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-400 text-[11px] font-bold uppercase tracking-wide">Save 16%</span>
            </button>
          </div>
        </div>

        {/* Pricing Cards Grid */}
        <div className="grid md:grid-cols-3 gap-6 max-w-[1050px] mx-auto items-start">
          
          {/* Starter / Free Tier */}
          <div className="bg-[#0a0a0a] rounded-[32px] p-10 flex flex-col border border-white/10 mt-0 md:mt-12">
            ...
            <button 
              onClick={() => onNavigate('app')}
              className="w-full py-3.5 bg-white/5 hover:bg-white/10 text-white rounded-full font-medium text-[16px] transition-colors mb-10 border border-white/5"
            >
              Try for free
            </button>
            <ul className="space-y-4">
              {[
                ...
                <li key={i} className="flex items-center gap-4 text-[15px] text-[#c0c0c0]">
                  <div className="w-5 h-5 rounded-full border border-[#444] flex items-center justify-center shrink-0">
          ...
          <div className="bg-[#151525] border border-indigo-500/30 rounded-[36px] flex flex-col p-2 relative shadow-[0_0_40px_rgba(99,102,241,0.08)]">
            ...
            <div className="bg-[#0a0a0a] rounded-[28px] p-8 flex flex-col flex-1 border border-white/5 shadow-2xl">
              ...
              <button 
                disabled
                className="w-full py-3.5 bg-indigo-600 text-white rounded-full font-medium text-[16px] transition-colors mb-10 opacity-50 cursor-not-allowed shadow-[0_8px_20px_rgba(79,70,229,0.3)]"
              >
                Coming soon
              </button>
              <ul className="space-y-4">
                {[
                  ...
                  <li key={i} className="flex items-center gap-4 text-[15px] text-[#c0c0c0]">
                    <div className="w-5 h-5 rounded-full border border-indigo-500/40 flex items-center justify-center shrink-0 bg-indigo-500/10">
          ...
          <div className="bg-[#0a0a0a] rounded-[32px] p-10 flex flex-col border border-white/10 mt-0 md:mt-12">
            ...
            <button 
              disabled
              className="w-full py-3.5 bg-white/5 text-white rounded-full font-medium text-[16px] transition-colors mb-10 border border-white/5 opacity-50 cursor-not-allowed"
            >
              Coming soon
            </button>
            <ul className="space-y-4">
...
console.log("Patched pricing UI");
