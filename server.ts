...
const DATABASE_URL = process.env.DATABASE_URL || 'postgresql://neondb_owner:npg_d9ZVKLESo6kO@ep-weathered-lab-ax9yxkm3-pooler.c-4.us-east-2.aws.neon.tech/neondb?sslmode=require&channel_binding=require';
...
setInterval(() => {
  ...
  for (const pin in sessions) {
    if (now - sessions[pin].lastActive > 1000 * 60 * 60) { // 1 hour timeout
      delete sessions[pin];
    }
  }
}, 1000 * 60 * 10);
...
async function startServer() {
  ...
  app.post('/api/sync/restore', (req, res) => {
    ...
    } else if (history && sessions[pin].history.length === 0) {
      sessions[pin].history = history;
    }
    
    res.json({ success: true, status: sessions[pin].status, history: sessions[pin].history });
  });
  ...
  app.post('/api/plugin/connect', (req, res) => {
  ...
  app.get('/api/plugin/poll/:pin', (req, res) => {
    const session = sessions[req.params.pin];
    if (!session) return res.status(404).json({ error: 'Session not found' });
  ...
  app.get('/api/sync/state/:pin', (req, res) => {
    const session = sessions[req.params.pin];
    if (!session) return res.status(404).json({ error: 'Session not found' });
  ...
  app.post('/api/plugin/state', (req, res) => {
  ...
  app.post('/api/chat', async (req, res) => {
    ...
    try {
      const session = pin ? sessions[pin] : null;
                    ...
                    if (edit.type === 'Icon') {
                        try {
                            const iconPrompt = (edit.prompt || edit.name || 'icon') + ' flat 2d vector icon solid green screen background';
                                ...
                                await execFileAsync('ffmpeg', ['-y', '-i', tempInput, '-vf', 'chromakey=0x00FF00:0.1:0.1', '-c:v', 'png', tempOutput]);
